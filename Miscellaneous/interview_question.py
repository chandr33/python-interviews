...
class FileIO(File): # search functions are out of scope, breaks the Single Responsibility Principle
    ...
    def search_file_by_file_size(self, size) -> List[str]: # Have these search functions as static methods or independent functions? Why
        ...
        for root, dirs, files in os.walk(path): # root, dirs, files not being used so better to replace them with _
            if os.stat(path + self.file_name).st_size == size:  # Use os.path.join
                list.append((os.path.join(self.file_name), stats.st_size))
            else:
                raise Exception("Nothing of size %d was found" % size) # Should throw a more specific exception, FileNotFound Exception, as well as be outside the loop
